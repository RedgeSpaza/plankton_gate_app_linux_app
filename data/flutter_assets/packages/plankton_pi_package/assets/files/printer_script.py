...
def print_text(printer, command):
    try:
        ...
        # Validate font_width and font_height
        ...
        printer.write(b'\x1B\x61' + bytes([alignments.get(align, 0)]))

        # Set bold and underline
        printer.write(b'\x1B\x45' + bytes([1 if bold else 0]))
        printer.write(b'\x1B\x2D' + bytes([1 if underline else 0]))

        if font_type.upper() == 'B':
            printer.write(b'\x1B\x4D\x01')  # Select Font B
        else:
            printer.write(b'\x1B\x4D\x00')  # Select Font A (default)

        size_byte = ((font_width - 1) << 4) | (font_height - 1)
        printer.write(b'\x1D\x21' + bytes([size_byte]))

        # Encode text using GB18030 encoding to support Chinese characters
        data = text.encode('gb18030', 'replace') + b'\n'

        # Send data in smaller chunks to prevent buffer overflow
