...
class DependencyInstaller:
    ...
    def setup(self):
        ...
        try:
            ...
            try:
                self.run_command(['sudo', 'rmmod', 'usblp'], check=False)
            ...
            try:
                print_json({"status": "progress", "message": "Setting up USB permissions..."})

                # Create udev rule for persistent permissions
                udev_rule = 'SUBSYSTEM=="usb", ATTRS{idVendor}=="4b43", ATTRS{idProduct}=="3830", MODE="0666"'
                udev_path = "/etc/udev/rules.d/99-printer.rules"
                ...
                print_json({"status": "progress", "message": "USB permissions set successfully"})

            except Exception as e:
                print_json({"status": "warning", "message": f"Failed to set USB permissions: {str(e)}"})
